...
const EntreeT = () => {
    ...
    // --------------------------------------------------------------
    // Construire la liste de toutes les demandes (CDF + USD)
    const allDemandes = getData
        ? getData
            .filter(item => item.montantCDF > 0 || item.montantUSD > 0)
            .map(item => ({
                ...item,
                devise: item.montantCDF > 0 ? 'CDF' : 'USD'
            }))
        : [];

    const totalItems = allDemandes.length;
    const totalPages = Math.ceil(totalItems / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const currentItems = allDemandes.slice(startIndex, startIndex + itemsPerPage);

    // Séparer les éléments de la page courante par devise
    const cdfItems = currentItems.filter(item => item.devise === 'CDF');
    const usdItems = currentItems.filter(item => item.devise === 'USD');
                ...
                <div className="col-md-7">
                    <div className="card border-0 shadow-sm rounded-3">
                        ...
                        <div className="card-body">
                            {getData !== undefined && (
                                <>
                                    {/* Affichage des demandes paginées */}
                                    {totalItems > 0 ? (
                                        <>
                                            {/* Tableau CDF */}
                                            {cdfItems.length > 0 && (
                                                <>
                                                    ...
                                                    <div className="table-responsive mb-4">
                                                        <table className="table table-hover table-sm">
                                                            ...
                                                            <tbody>
                                                                {cdfItems.map((res, index) => {
                                                                    const globalIndex = startIndex + index;
                                                                    return (
                                                                        <tr key={res.id}>
                                                                            <td className="fw-semibold">{globalIndex + 1}</td>
                                                                            ...
                                                                            <td>
                                                                                <div className="btn-group" role="group">
                                                                                    <button
                                                                                        ...
                                                                                        onClick={() => SaveDelestageCDF(res.id)}
                                                                                    >
                                                ...
                                                <>
                                                    ...
                                                    <div className="table-responsive">
                                                        <table className="table table-hover table-sm">
                                                            ...
                                                            <tbody>
                                                                {usdItems.map((res, index) => {
                                                                    const globalIndex = startIndex + cdfItems.length + index;
                                                                    return (
                                                                        <tr key={res.id}>
                                                                            <td className="fw-semibold">{globalIndex + 1}</td>
                                                                            <td>{res.NomDemandeur}</td>
                                                                            ...
                                                                            <td>
                                                                                <div className="btn-group" role="group">
                                                                                    <button
                                                                                        ...
                                                                                        type="button"
                                                                                        style={{ background: "#20c997", color: "white", borderRadius: "6px 0 0 6px" }}
                                                                                        onClick={() => SaveDelestageUSD(res.id)}
                                                                                    >
                                                                                        <i className="fas fa-check-circle me-1"></i> Délester
                                                                                    </button>
                                                                                    <button
                                            ...
                                            <div className="d-flex justify-content-between align-items-center mt-4 pt-2 border-top">
                                                <div className="d-flex align-items-center">
                                                    <span className="me-2 text-muted small">Lignes par page :</span>
                                                    <select
