...
//                             //     $creditRet->Gestionnaire,
//                             // );
//                            if ($this->checkRangeFonction($creditRet->NumDossier)==5){
//                                info("CHECK RANGE ".$this->checkRangeFonction($creditRet->NumDossier));
//                               $this->insertInTransactionRepriseProvision(
//                                 //round($soldeMembre, 2),
//                                 round($CapitalRestant, 2),
//                                 $creditRet->CodeMonnaie,
...
//                         //SI LA PROVISION EST 5 C A DIRE LE CREDIT VIENT A PEINE DE TOMBE EN RETARD ON CREDIT LE COMPTE 32
//                          if($provisionMatirute==5){
//                           Transactions::create([
//                             "NumTransaction" => $NumTransaction,
//                             "DateTransaction" => $dateSystem,
//                             "DateSaisie" => $dateSystem,
...
/* =========================
   PAR (ECHEANCIERS)
========================= */
$parQuery = DB::table('echeanciers as e')
    ->join('portefeuilles as p', 'e.NumDossier', '=', 'p.NumDossier')
    ->leftJoinSub($subRemboursement, 'r', function ($join) {
        $join->on('e.ReferenceEch', '=', 'r.RefEcheance');
    })
    ...
    ->selectRaw("
        ...
        SUM(CASE WHEN e.DateTranch IS NOT NULL AND DATEDIFF('$date', e.DateTranch) <= 0
            THEN (e.CapAmmorti - COALESCE(r.total_paye,0)) ELSE 0 END) AS EncoursSain,

        SUM(CASE WHEN e.DateTranch IS NOT NULL AND DATEDIFF('$date', e.DateTranch) BETWEEN 1 AND 30
            THEN (e.CapAmmorti - COALESCE(r.total_paye,0)) ELSE 0 END) AS PAR_1_30,

        SUM(CASE WHEN e.DateTranch IS NOT NULL AND DATEDIFF('$date', e.DateTranch) BETWEEN 31 AND 60
            THEN (e.CapAmmorti - COALESCE(r.total_paye,0)) ELSE 0 END) AS PAR_31_60,

        SUM(CASE WHEN e.DateTranch IS NOT NULL AND DATEDIFF('$date', e.DateTranch) BETWEEN 61 AND 90
            THEN (e.CapAmmorti - COALESCE(r.total_paye,0)) ELSE 0 END) AS PAR_61_90,

        SUM(CASE WHEN e.DateTranch IS NOT NULL AND DATEDIFF('$date', e.DateTranch) BETWEEN 91 AND 180
            THEN (e.CapAmmorti - COALESCE(r.total_paye,0)) ELSE 0 END) AS PAR_91_180,

        SUM(CASE WHEN e.DateTranch IS NOT NULL AND DATEDIFF('$date', e.DateTranch) > 180
            THEN (e.CapAmmorti - COALESCE(r.total_paye,0)) ELSE 0 END) AS PAR_PLUS_180
    ")
    ->groupBy('p.Gestionnaire');

/* =========================
   PORTFEUILLE (SOURCE STABLE)
========================= */
$portefeuilleQuery = DB::table('portefeuilles')
    ->selectRaw("
        Gestionnaire,
        COUNT(DISTINCT NumDossier) AS NbrCredits,
        SUM(MontantAccorde) AS TotalAccorde
    ")
...
========================= */
if (!empty($devise)) {
    $parQuery->where('p.CodeMonnaie', $devise);
    $portefeuilleQuery->where('CodeMonnaie', $devise);
}

if (!empty($gestionnaire) && $gestionnaire !== 'Tous') {
    $parQuery->where('p.Gestionnaire', $gestionnaire);
    $portefeuilleQuery->where('Gestionnaire', $gestionnaire);
}

/* =========================
   EXECUTION
========================= */
$parData = $parQuery->get()->keyBy('Gestionnaire');
...
/* =========================
   FUSION
========================= */
$data = $parData->map(function ($row, $gestionnaire) use ($portefeuilleData, $encoursGlobal) {
    ...
    $global = max($encoursGlobal, 0.0001);

    $row->TAUX_PAR_INTERNE = round(($row->PAR_SUP_1 / $encours) * 100, 2);
    $row->TAUX_PAR_GLOBAL = round(($row->PAR_SUP_1 / $global) * 100, 2);
...
$total = [
    ...
    'NbrCredits' => $portefeuilleData->sum('NbrCredits'),
    'TotalAccorde' => $portefeuilleData->sum('TotalAccorde'),
...
$pourcentageGlobal = round(
    ($parGlobal / max($encoursGlobal, 0.0001)) * 100,
    2
);
...
return response()->json([
    ...
    'data' => $data->values(),
    'total' => $total,
    'par_global_percent' => $pourcentageGlobal,
